					   ...
					   const int minz, const int maxz )
{
  ...
  if (parallel_flag) { 
    ...
    float view_width= lookdist * tan( 0.5*DegtoRad*fovea );
    ...
    data[0]= data[5]= 0.5*(float)screenwidth/(float)view_width;
    data[3]= 0.5*screenx;
    data[7]= 0.5*screeny;
    data[10]= ((float)zrange)/(hither-yon);
    ...
    // Objects far from the lookfm point have z values near minz!
    data[0]= data[5]= 0.5*(float)screenwidth / tan( 0.5*DegtoRad*fovea );
    data[2]= -0.5*screenx;
    data[6]= -0.5*screeny;
    data[10]= (((float)zrange)/(1.0 - (hither/yon))) - maxz;
    data[11]= ((-hither)*((float)zrange))/(1.0 - (hither/yon));
    data[14]= -1.0;
  }
