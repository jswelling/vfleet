...
#endif
#endif // CRAY_ARCH_T3D or CRAY_ARCH_T3E
#endif // AVOID_IMTOOLS
      owns_buffer= 1;
    }
    else {
...
rgbImage::rgbImage( FILE *fp, char *fname )
{
  ...
  if ( !(ImVfbQFields(buf) & IMVFBRGB) ) {
    ImVfb* new_buf= ImVfbToRgb(buf, IMVFBNEW);
    if ( new_buf==IMVFBNULL ) {
      image_valid= 0;
      TagTableFree( dataTable );
      TagTableFree( flagsTable );
      return;
  ...
  if (other->compressed()) {
	...
	if (*src_runs<0) {
	  ...
	  for (i=0; i<pix_count; i++) {
	    ...
	    // Black source pixels may occur on the T3D or T3E, because of the
	    // shmem mechanism for passing images.
	      ...
	      alphadif= alpha_dif_table[alpha_byte];
	  ...
	  // 128 cache lines long each.  Note that the alpha_dif_table
	  // array will screw things up somewhat, but so it goes.
...
#else // if ( CRAY_ARCH_T3D || CRAY_ARCH_T3E )
	  for (i=0; i<*src_runs; i++) {
